...
void tonemap(double *im, int N, int M, int K)

{
    ...
    for (int i = 0;i<N;i++)
        for (int j = 0;j<M;j++)
            for (int k = 0;k<3;k++)
            {
                MM = std::max(MM,im[i+j*N+k*N*M]);
                mm = std::min(mm,im[i+j*N+k*N*M]);
            }
...
void mexFunction(int nlhs, mxArray *plhs[], int nrhs, const mxArray *prhs[])
{
  ...
  if (L<3)
      mexErrMsgIdAndTxt("dtm_rgb:notrgbinput","Input should be an rgb-image.");
